...
host, port = '192.168.29.115', 64000
...
while(True):
    ...
    if (sensor.available()):
        ...
        green_dc = green_ac.averageDCEstimator(ir_ac.ir_avg_reg, ir)
        ...
        temperature = sensor.read_temperature() + 6

        # send the data to the TCP server
        current_time = time.ticks_ms() - start_time
        print(str(current_time) + " " + str(red) + " " + str(ir) + " " + str(green) + " " + str(average_heartbeat) + " " + str(average_spo2) + " " + str(temperature))
        # network_obj.sendTCPPacket(str(current_time) + " " + str(average_heartbeat) + " " + str(average_spo2) + " " + str(temperature) + " ")
        # network_obj.sendTCPPacket(str(current_time) + " " + str(red) + " " + str(ir) + " " + str(green) + " " + str(average_heartbeat) + " " + str(average_spo2) + " " + str(temperature) + "\n")
